    ...
    <div className="container mx-auto py-12 px-4">
      ...
      <div className="bg-white shadow rounded-lg p-6 mb-6">
        <h2 className="text-xl font-semibold mb-4 text-gray-700 flex items-center">
          Posts Test
          <span
            className={`ml-3 text-sm ${diagnostics.posts.status === "success" ? "text-green-600" : "text-red-600"}`}
          >
        ...
        {diagnostics.posts.data && (
          <div>
            <p className="text-gray-500 mb-2">
              Found {(diagnostics.posts.data as any[]).length} posts:
            </p>
      ...
      <div className="bg-white shadow rounded-lg p-6 mb-6">
        <h2 className="text-xl font-semibold mb-4 text-gray-700 flex items-center">
          Pages Test
          <span
            className={`ml-3 text-sm ${diagnostics.pages.status === "success" ? "text-green-600" : "text-red-600"}`}
          >
        ...
        {diagnostics.pages.data && (
          <div>
            <p className="text-gray-500 mb-2">
              Found {(diagnostics.pages.data as any[]).length} pages:
            </p>
            <ul className="list-disc list-inside space-y-1">
      ...
      <div className="bg-white shadow rounded-lg p-6 mb-6">
        <h2 className="text-xl font-semibold mb-4 text-gray-700 flex items-center">
          Site Settings Test
          <span
            className={`ml-3 text-sm ${diagnostics.settings.status === "success" ? "text-green-600" : "text-red-600"}`}
          >
            {diagnostics.settings.status === "success"
              ? "✓ Success"
              : "✗ Failed"}
        ...
        {diagnostics.settings.data && (
          <div className="space-y-2">
            <p className="text-gray-700">
              ...
              {(diagnostics.settings.data as any).title}
            </p>
            <p className="text-gray-700">
              <strong className="font-semibold">Description:</strong>{" "}
              {(diagnostics.settings.data as any).description}
            </p>
          </div>
        )}
      </div>
      ...
      <div className="bg-white shadow rounded-lg p-6 mb-6">
        <h2 className="text-xl font-semibold mb-4 text-gray-700 flex items-center">
          Menu Test (Primary)
          <span
            className={`ml-3 text-sm ${diagnostics.menu.status === "success" ? "text-green-600" : "text-red-600"}`}
          >
        ...
        {diagnostics.menu.error && (
          <div className="mb-4 p-4 bg-yellow-50 border border-yellow-200 rounded-lg text-yellow-800">
            ...
            <p className="text-sm mt-2 text-yellow-700">
              This is expected if you haven't installed the WP REST API Menus
              plugin yet.
            </p>
          </div>
        )}
      ...
      <div className="mt-8 p-6 bg-green-50 border border-green-200 rounded-lg text-green-800">
        ...
        <ol className="list-decimal list-inside space-y-2">
          <li>
            If you see errors above, check your <code>.env</code> file
          </li>
